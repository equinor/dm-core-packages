...
const TestUseListPlugin = (props: IUIPlugin) => {
	...
	return (
		<>
			...
			/>
			{attribute?.contained && <Button onClick={() => addItem()}>Add item</Button>}
				...
				{paginatedRows?.map((item: TItem<TGenericObject>) => {
					return (
						<li key={item.key}>
							...
							<Button
								onClick={() => updateItem(item, { ...item.data, name: 'NewName' })}
							>
								Update name
							</Button>
							<Button
								onClick={() => updateAttribute(item, 'complete', !item.data?.complete)}
							>
								Toggle complete
							</Button>
