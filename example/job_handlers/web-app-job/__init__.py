...
_SUPPORTED_TYPE = "dmss://DemoDataSource/apps/MySignalApp/models/SignalGeneratorAzureContainerJob"
...
class JobHandler(JobHandlerInterface):
    ...
    def __init__(self, job, data_source: str):
        ...
        print("********** calling cli")
        # print(az_cli(f"vm list -g {config.AZURE_JOB_RESOURCE_GROUP}"))
        # print(az_cli('cloud list'))
        print(get_default_cli().invoke(['login']))
        print("*****")
        ####
    ...
    def start(self) -> str:
        logger.info(f"JobName: '{str(self.job.job_uid)}'. Starting Azure Container job...")
        ...
        runner_entity: dict = self.job.entity.get("runner", None)
        ...
        # TODO make the container job update signal value in the entity.
    ...
    def result(self) -> Tuple[str, bytes]:
        job_status = self.job.status
        return f"Status: {job_status}", b"12345"
