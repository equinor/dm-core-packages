...
test('Locations', async () => {
  const locations = page.getByTestId('locations')
  await expect(locations.getByRole('textbox')).toHaveCount(1)
  await expect(locations.getByRole('textbox')).toHaveValue('Trondheim')
  await locations.getByRole('button', { name: 'Add' }).click()
  await expect(locations.getByRole('textbox')).toHaveCount(2)
  await locations.getByRole('textbox').last().fill('Oslo')
  await page.getByRole('button', { name: 'Submit' }).click()
  await page.reload()
  await navigate()
  await expect(locations.getByRole('textbox')).toHaveCount(2)
  await expect(locations.getByRole('textbox').first()).toHaveValue('Trondheim')
  await expect(locations.getByRole('textbox').last()).toHaveValue('Oslo')
  await locations.getByRole('button', { name: 'Remove' }).last().click()
  await expect(locations.getByRole('textbox')).toHaveCount(1)
  await page.getByRole('button', { name: 'Submit' }).click()
  await page.reload()
  await navigate()
...
test('New car', async () => {
  const cars = page.getByTestId('cars')
  await expect.soft(cars.getByText('1 - 2 of 2')).toBeVisible()
  await cars.getByRole('button', { name: 'Append Add Item' }).click()
  await expect.soft(cars.getByText('1 - 3 of 3')).toBeVisible()
  await cars.getByRole('button', { name: 'Save' }).click()
  await cars.getByRole('button', { name: 'Open item' }).last().click()
