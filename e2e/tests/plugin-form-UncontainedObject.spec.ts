...
test('uncontainedObject', async ({ page }) => {
  ...
  await test.step('Add assistant', async () => {
    ...
    await page
      ...
      .click()
    await expect(page.getByTestId('assistant').getByRole('code').getByText('John')).toBeVisible()
    await expect(page.getByTestId('assistant').getByRole('code').getByText('1234')).toBeVisible()
