					...
					if (attribute.name === 'runner') {
						return (
							<>
								...
								<Button onClick={() => setShowJobRunnerModal(true)}>Select</Button>
								<EntityPickerDialog
								...
								/>
								<Typography>
									{formData?.runner && 'Selected: ' + JSON.stringify(formData.runner)}
								</Typography>
							</>
						)
					}
								...
								<div>
									...
									<Typography>
										{formData?.applicationInput &&
											'Selected: ' + JSON.stringify(formData.applicationInput.address)}
									</Typography>
								</div>
					...
					return (
						<>
							<TextField
								...
								label={attribute.name + (attribute.optional ? ' (optional)' : '')}
								...
								value={formData[attribute.name as keyof TJob] as string | number}
							/>
						</>
					)
				})}
