...
export const DatasourceApiFp = function(configuration?: Configuration) {
    ...
    return {
        ...
        async dataSourceGet(dataSourceId: string, options?: AxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<object>> {
            ...
            const operationBasePath = operationServerMap['DatasourceApi.dataSourceGet']?.[index]?.url;
        ...
        async dataSourceGetAll(options?: AxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<Array<DataSourceInformation>>> {
            ...
            const index = configuration?.serverIndex ?? 0;
            const operationBasePath = operationServerMap['DatasourceApi.dataSourceGetAll']?.[index]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, operationBasePath || basePath);
        },
        /**
         * Create or update a data source configuration.  This endpoint is used for creating or updating a data source configuration. A data source can have multiple repositories.  Args: - data_source_id (str): ID of the data source - new_data_source (DataSourceRequest): A dict object with keys \"name\" and \"repositories\" which is another dict of str and repository configuration. This is the config of the data source. - user (User): The authenticated user accessing the endpoint, automatically generated from provided bearer token or Access-Key.   Returns: - str: The ID of the newly created or updated data source.
        ...
        async dataSourceSave(dataSourceId: string, dataSourceRequest: DataSourceRequest, options?: AxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<string>> {
            ...
            const index = configuration?.serverIndex ?? 0;
            const operationBasePath = operationServerMap['DatasourceApi.dataSourceSave']?.[index]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, operationBasePath || basePath);
        },
    }
};
