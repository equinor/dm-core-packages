				...
				{config.columns.map((column: TTableColumnConfig) => {
					...
					return (
						<SortCell
							...
							key={column.data}
							isSorted={
								column.data === sortColumn && tableVariant === TableVariantNameEnum.View
							}
							onClick={() => (column.sortable ? sortByColumn(column.data) : null)}
							tabIndex={column.sortable ? 0 : -1}
						>
							{column.label || column.data}
							{column.sortable && tableVariant === TableVariantNameEnum.View && (
								<Icon
									name={sortDirection === 'descending' ? 'arrow_down' : 'arrow_up'}
								/>
							)}
						</SortCell>
					)
